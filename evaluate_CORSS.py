...
def evaluate_single_scenario(model, tau, max_targets, max_measurements, device, output_dir):
    ...
    # 生成RMSE图
    plot_rmse(all_rmse, tau, output_dir)
...
def visualize_scenario(trajectories, measurements, tracked_states, tracked_measurements, 
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'crossing_trajectories.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  保存轨迹可视化: {output_path}")


def plot_rmse(rmse_values, tau, output_dir):
    """绘制RMSE曲线"""
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'crossing_rmse.png')
...
def main():
    ...
    # 保存结果
    summary_results = {
        'scenario_type': 'fixed_crossing_trajectories',
        'num_trajectories': 2,
        'num_frames': 40,
        'avg_association_accuracy': float(results['avg_association_accuracy']),
        'avg_rmse': float(results['avg_rmse']),
        'description': 'Simple crossing scenario with 2 trajectories intersecting at center'
    }
    
    results_path = os.path.join(args.output_dir, 'crossing_evaluation_summary.json')
