...
for t in trajs:
    print(f'  label={t["label"]} birth={t["birth_frame"]} death={t["death_frame"]}')
...
print(f'GRU loaded: {mgr._use_gru}')
print(f'gru_confirm_thresh={cfg.gru_confirm_thresh}  '
      f'gru_min_age_confirm={cfg.gru_min_age_confirm}  '
      f'gru_delete_thresh={cfg.gru_delete_thresh}  '
      f'gru_delete_streak={cfg.gru_delete_streak}')
...
with torch.no_grad():
    for frame_idx in range(len(meas)):
        ...
        active = [t for t in mgr.tracks.values() if t.status in ('tentative', 'confirmed')]
        palive_strs = []
        for t in active[:8]:
            p = t.p_alive if t.p_alive is not None else -1.0
            palive_strs.append(f'{p:.2f}({t.status[0]})')
        palive_str = ' '.join(palive_strs)

        n_conf2 = len(mgr.get_confirmed_tracks())
        n_tent2 = sum(1 for t in mgr.tracks.values() if t.status == 'tentative')
        print(f'Frame {frame_idx:2d}: tent={n_tent2} conf={n_conf2} '
              f'n_past={n_past if frame_idx>=tau else "-"} | {palive_str}')
